...
class GameTestCase(unittest.TestCase):
    def setUp(self):
        self.controller = game.controller.test_client()

    def tearDown(self):
        pass

    def test_should_start_new_game(self):
        response = self.controller.post('/start')
    ...
    def test_should_return_game_info(self):
        response = self.controller.get('/123')
        self.assertEqual(200, response.status_code)
        self.assertEqual(json.dumps({"name": "Number Guess", "id": 123}, indent = 2), response.data)

    def test_should_return_too_high(self):
        response = self.controller.post('/123/guesses', data = dict(guess=7))
        self.assertEqual(200, response.status_code)

    def test_should_return_too_low(self):
        response = self.controller.post('/123/guesses', data = dict(guess=3))
        self.assertEqual(200, response.status_code)

    def test_should_return_match(self):
        response = self.controller.post('/123/guesses', data = dict(guess=5))
        self.assertEqual(200, response.status_code)

    def test_should_end_inprogress_game(self):
        response = self.controller.post('/123/end')
        self.assertEqual(200, response.status_code)
